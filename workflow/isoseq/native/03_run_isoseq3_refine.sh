...
# Stage 03 · refine（isoseq3：polyA/接头去除，对应 03ISOSEQ_REFINE）

# ---- 路径自锚定：以脚本位置解析仓库根与包装器（保证任意机器/工作目录可运行）----
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "${SCRIPT_DIR}/../../.." && pwd)"
ISOSEQ3_REFINE=${ISOSEQ3_REFINE:-"${REPO_ROOT}/modules/isoseq3/native/isoseq3_refine.py"}   # isoseq3 refine 包装器（可环境变量覆盖）

# 可选：激活运行环境
# conda activate pacbio_iso_seq

# 配置参数（可通过环境变量覆盖）
DATA_DIR=${DATA_DIR:-""}                # 必填：指向包含样本子目录/lima 结果的路径
OUT_BASE=${OUT_BASE:-"isoseq3_refine_output"}   # 输出根目录（默认当前目录下 isoseq3_refine_output）
...
ISOSEQ3_BIN=${ISOSEQ3_BIN:-""}          # 可选：isoseq3 可执行文件路径（留空则从 PATH 解析）

if [[ -z "$DATA_DIR" || ! -d "$DATA_DIR" ]]; then
  echo "[ERROR] 未设置或不存在 DATA_DIR。请导出：export DATA_DIR=/path/to/lima_output" >&2
  exit 1
fi
...
while IFS= read -r bam; do
  ...
  mkdir -p "$outdir"
  cmd_line="python3 \"${ISOSEQ3_REFINE}\" --bam \"$bam\" --primers \"$PRIMERS\" --outdir \"$outdir\" --cpus \"$CPUS_PER_TASK\""
